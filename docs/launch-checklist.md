...
- Abrir `/studio` y publicar en este orden: categorías, perfil fundador, lugares, artículos y guía.
...
- Ejecutar `001_initial_schema.sql` y después `002_launch_foundation.sql` en el SQL Editor.
...
- Confirmar que los roles `anon` y `authenticated` no pueden leer la tabla.
...
- Confirmar pageviews y los eventos `article_view`, `article_read_75`, `place_view`, `guide_view`, `category_filter` y `newsletter_submit`.
...
- Hacer una revisión móvil y de escritorio de portada, guía, artículo, lugar, Studio y privacidad.
